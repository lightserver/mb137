...
object Frontend extends js.JSApp {
  ...
  println("starting ping frontend")
  ...
  def joinChat(name: String): Unit = {
    ...
    chat.onmessage = { (event: MessageEvent) ⇒
      val wsMsg = read[PingMessage](event.data.toString)
      playground.insertBefore(p(s" serversaid: ${wsMsg.message}"), playground.firstChild)
    }
